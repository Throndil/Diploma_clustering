...
for (i in 1:number_of_clusters) {
  for (j in 1:length(mydata.clusters)) {
    ...
    if (j == length(mydata.clusters) && rows_in_cluster == 1) {
    #  message("Anomalia v zhluku: ", i, ", na indexe: ", row_index)
      mydata.clusters <- mydata.clusters[-c(row_index)]
      vectors_matrix <- vectors_matrix[-row_index,]
    }
  }
  rows_in_cluster = 0
  row_index = 0
...
#write.csv(mydata.clusters, "pilot_project_10shift_50width_clusters.csv", row.names = FALSE, col.names = FALSE)
###### UKLADANIE SUBOROV AKO TREBA BEZ RIADKOV STLPCOV, Z MATICE NA DATFRAME NA MATICU
table(mydata.clusters)
length(mydata.clusters)
mydata.clusters_dataframe = as.data.frame(mydata.clusters)
names(mydata.clusters_dataframe) <- NULL
mydata.clusters_matrix_test = as.matrix(mydata.clusters_dataframe)
#write.table(mydata.clusters_matrix_test, "pilot_project_10shift_50width_clustersv2.csv", row.names = FALSE, col.names = FALSE)

write.csv(vectors_matrix, "pilot_project_10shift_50width_heatmap_vectorsv2.csv", row.names = FALSE,  col.names = FALSE)
#write.table(vectors_matrix, "pilot_project_10shift_50width_heatmap_vectors.csv", row.names = FALSE,  col.names = FALSE)

vector_matrix_test = as.data.frame(vectors_matrix)
names(vector_matrix_test) <- NULL
#write.table(vector_matrix_test, "pilot_project_10shift_50width_heatmap_vectors_dataframe.csv", row.names = FALSE,  col.names = FALSE)
vector_matrix_matrix_test = as.matrix(vector_matrix_test)
write.csv(vector_matrix_test, "pilot_project_10shift_50width_heatmap_vectors.csv", row.names = FALSE,  col.names = FALSE)



length(vectors_matrix)
dim(vectors_matrix)
vectors_matrix[236,]
## su za sebou a je automaticke posunutie indexov takze musim zmazat zase ten isty index

graph <- fviz_cluster(list(data=vectors_matrix, cluster = mydata.clusters), stand = FALSE, labelsize = 0)
graph
## DRUHE ZHLUKOVANIE
table(mydata.clusters)
mydata_data_std = vectors_matrix
...
for (i in 1:number_of_clusters) {
  ...
  if (is.null(nrow(v))) {
    ...
    clusters_list[[i]] = d
    clusters_matrix = matrix(unlist(clusters_list[[i]]), ncol=nrow(d), nrow=1, byrow=TRUE)
...
## POZERANIE PRE BODY MIMO POLOMERU
v = y[z == 10,]
nrow(v)
c = colMeans(v)
u = t(v) - c
